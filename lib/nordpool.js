...
const parseResult = (data, opts) => {
  ...
  if (data.data && data.data.Rows && data.data.Rows.length) {
    for (var i = 0; i < data.data.Rows.length; i++) {
      ...
      const startsAt = dayjs.tz(row.StartTime, "YYYY-MM-DD\Thh:mm:ss", 'Europe/Oslo').tz();
...
const fetchPrices = async (aDate, opts) => {
  try {
    const ops = [
      aDate.$offset > 120 ? getHourlyPrices(aDate.add(-1, 'day'), opts) : undefined,
      getHourlyPrices(aDate, opts),
      getHourlyPrices(aDate.add(1, 'day'), opts)
    ];
