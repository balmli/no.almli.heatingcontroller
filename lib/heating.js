...
let isDay = function (aDate, opts) {
    ...
    if (workDay) {
        return opts.workday.startHour <= opts.workday.endHour ?
            hour >= opts.workday.startHour && hour < opts.workday.endHour :
            hour >= opts.workday.startHour || hour < opts.workday.endHour;
    }

    return opts.notWorkday.startHour <= opts.notWorkday.endHour ?
        hour >= opts.notWorkday.startHour && hour < opts.notWorkday.endHour :
        hour >= opts.notWorkday.startHour || hour < opts.notWorkday.endHour;
};
