...
let isWorkDay = function (aDate) {
    return !isHoliday(aDate) && aDate.getDay() >= 1 && aDate.getDay() <= 5;
};

let isWorkTime = function (aDate, opts) {
    let hour = aDate.getHours() + aDate.getMinutes() / 60 + aDate.getSeconds() / 3600;
    return isWorkDay(aDate) && hour >= opts.workHours.startHour && hour <= opts.workHours.endHour;
};

let isHoliday = function (aDate) {
    let today = new Date();
