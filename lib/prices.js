...
const pricesSorted = (prices, aDate) => {
  return pricesStarting(prices, aDate, 0, 24)
    .concat()
    .sort((a, b) => a.price - b.price);
};
...
const priceLevel = (prices, aDate) => {
  const price = currentPrice(prices, aDate);
  const averagePrice = averagePricesStarting(prices, aDate, 0, 24);
...
const priceHighLow = (prices, aDate) => {
  const sorted = pricesSorted(prices, aDate);
...
const priceNextHours = function (prices, aDate, num_hours) {
...
const currentPriceLowerThanNext = (prices, aDate, numHours) => {
  const pricesFollowing = priceNextHours(prices, aDate, numHours);
  if (pricesFollowing.length === 0) {
    return false;
  }

  const minOfPeriod = minOfHighestPrices(pricesFollowing, numHours);
...
const currentPriceHigherThanNext = (prices, aDate, numHours) => {
  const pricesFollowing = priceNextHours(prices, aDate, numHours);
  if (pricesFollowing.length === 0) {
    return false;
  }

  const maxOfPeriod = maxOfLowestPrices(pricesFollowing, numHours);
...
module.exports = {
  ...
  checkSumPrices,
  currentPriceLowerThanNext,
  currentPriceHigherThanNext,
};
