...
const priceRatio = (prices, aDate) => {
  const priceNow = currentPrice(prices, aDate);
  if (!priceNow) {
    return;
  }
  ...
  const min = Math.min(...pricesToday);
  const max = Math.max(...pricesToday);
  return Math.round(((priceNow.price - min) / (max - min)) * 1000000) / 1000000;
};
