...
function ephemeris(jd, earth) {
  ...
  var K = 73.6667 * Math.PI / 180 + 1.3958333 * Math.PI / 180 * (jd - 2396758) / _base2.default.JulianCentury;

  var solarPos = _solar2.default.trueVSOP87(earth, jd);
  var L = solarPos.lon;
  var R = solarPos.range;

  var _nutation$nutation = _nutation2.default.nutation(jd),
      _nutation$nutation2 = _slicedToArray(_nutation$nutation, 2),
      Δψ = _nutation$nutation2[0],
      Δε = _nutation$nutation2[1];

  var ε0 = _nutation2.default.meanObliquity(jd);
  var ε = ε0 + Δε;
  var λ = L - 20.4898 / 3600 * Math.PI / 180 / R;
  var λp = λ + Δψ;

  var _base$sincos = _base2.default.sincos(λ - K),
      _base$sincos2 = _slicedToArray(_base$sincos, 2),
      sλK = _base$sincos2[0],
      cλK = _base$sincos2[1];

  var _base$sincos3 = _base2.default.sincos(I),
  ...
  var L0 = _base2.default.pmod(η - θ, 2 * Math.PI);
  return [P, B0, L0];
}
